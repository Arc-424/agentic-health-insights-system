...
def generate_analysis(data, system_prompt, check_only=False, session_id=None):
    ...
    # Legacy path: direct analysis agent call (used by analysis_form directly)
    return st.session_state.analysis_agent.analyze_report(
        data=data, system_prompt=system_prompt, check_only=False
    )


def run_agent_pipeline(patient_meta: dict, pdf_file=None, use_sample: bool = False) -> dict:
    """
    Execute the full agentic pipeline via AgentOrchestrator.

    Returns orchestrator result dict:
        {success, insight, validation_result, retries, error}
    """
    init_analysis_state()

    # Rate-limit check before running the pipeline
    can_run, error_msg = st.session_state.analysis_agent.check_rate_limit()
    if not can_run:
        return {"success": False, "insight": "", "validation_result": {}, "retries": 0, "error": error_msg}
    ...
    if result["success"]:
        st.session_state.analysis_agent._update_analytics({"model_used": "orchestrator"})
